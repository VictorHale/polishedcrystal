if DEF(FAITHFUL)
	db  90,  75,  85,  55, 115,  90 ; 510 BST
	;   hp  atk  def  spd  sat  sdf
else
	db  95,  75,  90,  65, 125,  95 ; 545 BST
	;   hp  atk  def  spd  sat  sdf
endc

if DEF(FAITHFUL)
	db ELECTRIC, ELECTRIC ; type
	db 45 ; catch rate
	db 194 ; base exp
else
	db ELECTRIC, DRAGON ; type
	db 40 ; catch rate
	db 204 ; base exp
endc
	db NO_ITEM ; item 1
	db NO_ITEM ; item 2
	dn GENDER_F50, 3 ; gender ratio, step cycles to hatch
	INCBIN "gfx/pokemon/ampharos/front.dimensions"
if DEF(FAITHFUL)
	abilities_for AMPHAROS, STATIC, STATIC, MOLD_BREAKER
else
	abilities_for AMPHAROS, STATIC, DRIZZLE, MOLD_BREAKER
endc
	db GROWTH_MEDIUM_SLOW ; growth rate
	dn EGG_MONSTER, EGG_GROUND ; egg groups

	ev_yield   0,   0,   0,   0,   3,   0
	;         hp  atk  def  spd  sat  sdf

	; tm/hm learnset
	tmhm DYNAMICPUNCH, CURSE, TOXIC, HIDDEN_POWER, HYPER_BEAM, LIGHT_SCREEN, PROTECT, RAIN_DANCE, SAFEGUARD, BULLDOZE, GILDED_SLASH, THUNDERBOLT, THUNDER, RETURN, ROCK_SMASH, DOUBLE_TEAM, FLASH_CANNON, SWIFT, SUBSTITUTE, FACADE, REST, ATTRACT, DAZZLINGLEAM, FOCUS_BLAST, WILD_CHARGE, DRAGON_PULSE, GIGA_IMPACT, FLASH, VOLT_SWITCH, PLASTIZE, STRENGTH, BODY_SLAM, COUNTER, DEFENSE_CURL, DOUBLE_EDGE, ENDURE, FIRE_PUNCH, HEADBUTT, SEISMIC_TOSS, SLEEP_TALK, SWAGGER, THUNDERPUNCH, ZAP_CANNON
	; end
